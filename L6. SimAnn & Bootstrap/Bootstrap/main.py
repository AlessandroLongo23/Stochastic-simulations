...
if __name__ == "__main__":
    ...
    # means, medians, stds = bootstrap.estimate(X_i, n_bootstrap=10000)
    # Ys = [sum(X_i) / len(X_i) - means[i] for i in range(n)]
    ...
    # Ss_squared = [np.sum(X_i - means[i]) ** 2 / (n - 1) for i in range(n)]
    # std_Ss_squared = np.std(Ss_squared)

    # print(f"Mean: {np.mean(Ss_squared)}, Std: {std_Ss_squared}")
    ...
    print(f'Variance of the mean (bootstrap): {np.std(means) ** 2}')
